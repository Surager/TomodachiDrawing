...
class ControllerServer():
    ...
    def mainloop(self, itr, ctrl):
        ...
        while True:
            ...
            try:
                # The "Change Grip/Order" menu is sensitive to packet
                # flooding, so before we leave it we keep the original
                # dedupe behaviour: only send when the payload changes,
                # plus a slow heartbeat to keep the connection alive.
                #
                # Once the controller is in-game (Grip/Order menu has
                # been exited), deduping identical payloads is harmful.
                # A single dropped HID packet on an "edge" frame
                # (e.g. A release, neutral, direction change) would
                # otherwise leave a button stuck until either the next
                # state change or the ~1s (132 tick) heartbeat. Real
                # Pro Controllers continuously re-assert their current
                # state at ~120Hz, so we mirror that and send on every
                # tick; this makes single-frame state changes resilient
                # to BT-link drops and prevents the "A sticks during
                # drawing -> dragged line" / "missed pixel desyncs the
                # whole row" failures.
                if msg[3:] != self.cached_msg:
                    self.cached_msg = msg[3:]
                    itr.sendall(msg)
                elif self.input.exited_grip_order_menu:
                    itr.sendall(msg)
                elif self.tick >= 132:
                    itr.sendall(msg)
                    self.tick = 0
