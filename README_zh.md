...
**第三方组件：** 本仓库包含对 [Brikwerk/nxbt](https://github.com/Brikwerk/nxbt) 的**修改版**源码（MIT 许可），说明见仓库根目录 [NOTICE](NOTICE) 及 [`third_party/nxbt/`](third_party/nxbt/)（含 `LICENSE`、`PATCHES.md`）。
...
Python 源码集中在 **`tomodachi_drawing/`** 包内；仓库根目录保留本仓库许可证与 **`NOTICE`**、依赖说明与 README。
...
**修改版 nxbt** 位于 **`third_party/nxbt/`**（内含 Python 包 `nxbt/`、上游 `LICENSE`、本地变更说明 `PATCHES.md`）。运行时代码通过 `tomodachi_drawing.nxbt_path` 加载；可用环境变量 **`NXBT_SOURCE_DIR`** 指向其他含 `nxbt` 包的目录以覆盖默认路径。
...
- `tomodachi_drawing.ctrl` 依赖 vendored `nxbt`（见 `NOTICE`）与 `tqdm`。
